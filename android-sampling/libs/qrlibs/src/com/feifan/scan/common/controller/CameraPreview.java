...
public class CameraPreview extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void init(Camera camera, Camera.PreviewCallback previewCallback) {
        ...
        mTargetHeight = getHeight();
        mTargetWidth = getWidth();
    }
    ...
    /**
     * 在处理preview的帧图片的时候有时候会出现的内存溢出，此时降低采样的尺寸，降低的标准是宽度，因为高度会由系统自动控制
     * @param width
     */
    public void setTargetWidth(int width){
        if(width > 0 && mCamera != null){
            List<Camera.Size> sizes = mCamera.getParameters().getSupportedPreviewSizes();
            if(sizes != null && sizes.size() > 0) {
                int minWidth = sizes.get(0).width;
                ...
                mTargetWidth = width;
                if(getHeight() != 0) {
                    double targetRatio = (double) getWidth() / getHeight();
                    if (targetRatio > 0) {
                        mTargetHeight = (int) (mTargetWidth / targetRatio);
...
//        int w = getWidth();
//        int h = getHeight();
        int w = mTargetWidth == 0?getWidth():mTargetWidth;
        int h = mTargetHeight == 0?getHeight():mTargetHeight;
