...
public class IBeacon implements Parcelable{
	...
	private int mInsideNum = 3;
	private final int INSIDE_NUM = 3;
	private final int INSIDE_DISTANCE = 3;
	...
	public String getProximityUuid() {
		return proximityUuid.toString().trim();
	}
	
	
	@Override
	public int hashCode() {
		return minor+major+getProximityUuid().hashCode();
	}
	...
	public static IBeacon fromScanData(ScanData scanData) {
		if(scanData != null && scanData.scanRecord != null){
			...
			if(scanData.device != null){
				mac = scanData.device.getAddress();
			}
			IBeacon iBeacon = fromScanData(scanData.scanRecord,scanData.rssi);
			if (iBeacon != null) {
				iBeacon.setMac(mac);
				return iBeacon;
			}
	...
	public static IBeacon fromScanData(byte[] scanData, int rssi) {
		...
		iBeacon.setTime(System.currentTimeMillis());
		return iBeacon;
	}

	private static void debug(byte[] scanData){
		byte[] proximityUuidBytes = new byte[16];
		System.arraycopy(scanData, 9, proximityUuidBytes, 0, 16);
		String hexString = bytesToHex(proximityUuidBytes);
		StringBuilder sb = new StringBuilder();
	...
	public void insidePlus(){
		if(mInsideNum < INSIDE_NUM) {
			mInsideNum++;
		}
	}

	public void insideSub(){
		if(mInsideNum >0) {
			mInsideNum --;
		}else {
			mInsideNum = 0;
	...
	public boolean isOutSide(){
		return mInsideNum == 0 ? true:false;
	}

	public void checkOutSide(){
		if(getAccuracy() > INSIDE_DISTANCE){
			insideSub();
		}
	}
	...
	public void copyBeaconInsideNum(IBeacon iBeacon){
		if(iBeacon != null){
			this.mInsideNum = iBeacon.getBeaconInside();
		}
	}
