 ...
 * under the License.
 */
package com.feifan.beacon.service;
...
import com.feifan.beacon.BuildConfig;
import com.feifan.beacon.Region;
import com.feifan.beacon.distance.DistanceCalculator;
import com.feifan.beacon.distance.ModelSpecificDistanceCalculator;
import com.feifan.beacon.logging.LogManager;
import com.feifan.beacon.service.scanner.CycledLeScanCallback;
import com.feifan.beacon.service.scanner.CycledLeScanner;
import com.feifan.beacon.service.scanner.NonBeaconLeScanCallback;
import com.feifan.beacon.startup.StartupBroadcastReceiver;
import com.feifan.bluetooth.BluetoothCrashResolver;
...
public class BeaconService extends Service {
     ...
     * restart scanning, it is not instantaneous, and you lose any com.my.com.my.com.mm.beacon packets that were in the
     ...
     * of seconds, because then it may always miss a com.my.com.my.com.mm.beacon that is synchronized with when it is stopping
     * scanning.
     *
     */
            ...
            if (service != null) {
                switch (msg.what) {
                    case MSG_START_RANGING:
                        ...
                        service.startRangingBeaconsInRegion(startRMData.getRegionData(), new com.feifan.beacon.service.Callback(startRMData.getCallbackPackageName()));
                    ...
                    case MSG_START_MONITORING:
                        ...
                        service.startMonitoringBeaconsInRegion(startRMData.getRegionData(), new com.feifan.beacon.service.Callback(startRMData.getCallbackPackageName()));
                        service.setScanPeriods(startRMData.getScanPeriod(), startRMData.getBetweenScanPeriod(), startRMData.getBackgroundFlag());
                        break;
    ...
    public void onCreate() {
        ...
        try {
            Class klass = Class.forName("org.altbeacon.com.my.com.my.com.mm.beacon.SimulatedScanData");
            java.lang.reflect.Field f = klass.getField("beacons");
            this.simulatedScanData = (List<Beacon>) f.get(null);
        } catch (ClassNotFoundException e) {
            LogManager.d(TAG, "No org.altbeacon.com.my.com.my.com.mm.beacon.SimulatedScanData class exists.");
        } catch (Exception e) {
            LogManager.e(e, TAG, "Cannot get simulated Scan data.  Make sure your org.altbeacon.com.my.com.my.com.mm.beacon.SimulatedScanData class defines a field with the signature 'public static List<Beacon> beacons'");
        }
    }
            ...
            if (BeaconManager.getBeaconSimulator() != null) {
                ...
                if (BeaconManager.getBeaconSimulator().getBeacons() != null) {
                    if (0 != (getApplicationInfo().flags &= ApplicationInfo.FLAG_DEBUGGABLE)) {
                        ...
                        LogManager.w(TAG, "Beacon simulations provided, but ignored because we are not running in debug mode.  Please remove com.my.com.my.com.mm.beacon simulations for production.");
                    }
                } else {
    ...
    private void processBeaconFromScan(Beacon beacon) {
        ...
        if (LogManager.isVerboseLoggingEnabled()) {
            LogManager.d(TAG,
                    "com.my.com.my.com.mm.beacon detected : %s", beacon.toString());
        }

        beacon = mGattBeaconTracker.track(beacon);
        // If this is a Gatt com.my.com.my.com.mm.beacon that should be ignored, it will be set to null as a result of
        // the above
        if (beacon == null) {
            if (LogManager.isVerboseLoggingEnabled()) {
                LogManager.d(TAG,
                        "not processing detections for GATT extra data com.my.com.my.com.mm.beacon");
            }
        } else {
            ...
            LogManager.d(TAG, "looking for ranging region matches for this com.my.com.my.com.mm.beacon");
            synchronized (rangedRegionState) {
    ...
    private List<Region> matchingRegions(Beacon beacon, Collection<Region> regions) {
        ...
        for (Region region : regions) {
            if (region.matchesBeacon(beacon)) {
                ...
                LogManager.d(TAG, "This region (%s) does not match com.my.com.my.com.mm.beacon: %s", region, beacon);
            }
        }
        return matched;
    }
