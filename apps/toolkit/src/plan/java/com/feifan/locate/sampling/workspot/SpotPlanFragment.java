 ...
 */
public class SpotPlanFragment extends AbsSensorFragment implements OnPlanListener, OnOperationCallback {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        markLayer.setCallback(this);
        plan.addLayer(markLayer);

//        String[] items = getResources().getStringArray(R.array.menu);
        menu = new BubbleMenu(getContext());
        menu.addItem(ID_CONFIRM, "确定并开始采集");
        menu.setOnItemListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                switch (v.getId()) {
                    case ID_CONFIRM:
                        if(currentPoint != null && currentPoint instanceof IPlanPointImpl) {
                            currentPoint.setMovable(false);
...
//        board.setText(value);
        ...
        switch (requestCode) {
            case REQUEST_CODE_DETAIL:
                if(resultCode == Activity.RESULT_OK) {
                    // 删除标记点
                    IPlanPointImpl mark = data.getParcelableExtra(SpotDetailFragment.EXTRA_KEY_MARKPOINT);
    ...
    protected int getLoaderId() {
        return Constants.LOADER_ID_WORKSPOT;
    }
    ...
    private void loadSpots(Cursor data) {
        ...
        if(data != null && data.getCount() != 0) {
            List<IPlanPointImpl> marks = new ArrayList<IPlanPointImpl>();
            while(data.moveToNext()) {
                ...
                IPlanPointImpl point = new IPlanPointImpl(0, 0, x, y, zone.scale);
    ...
    public ILayerPoint onCreatePoint(PointInfo info) {
        ...
        if(cursor.getCount() == 0) { // 图中没有可移动点
            ...
            // 创建坐标点
            retPoint = new IPlanPointImpl(id, roundX, roundY);
